...
function requireValidSeriesId(value: string) {
  if (!/^\d+$/.test(value)) {
    console.warn(`Rejected invalid seriesId: ${value}`);
    notFound();
  }
...
function findSeriesName(items: WorkItem[], seriesId: string) {
  for (const item of items) {
    const seriesList = Array.isArray(item.iteminfo?.series)
      ? item.iteminfo.series
      : [];
    const series = seriesList.find(
      (candidate) =>
        String(candidate.id ?? candidate.series_id ?? "") === seriesId
    );
...
const getSeriesPageData = cache(
  async (seriesId: string, page: number, sort: WorkSort) => {
    ...
    const seriesName = findSeriesName(items, seriesId);

    return {
      ...result,
      seriesName,
      title: seriesName
        ? `${seriesName}作品一覧 - AV大臣`
        : `シリーズID ${seriesId} の作品一覧 - AV大臣`,
    };
  }
);
...
export async function generateMetadata({
  ...
  const seriesId = requireValidSeriesId(id);
  ...
  const { dataStatus, title, totalCount } = await getSeriesPageData(
    seriesId,
    currentPage,
    currentSort
  );
  ...
  return {
    ...
    alternates: {
      canonical: createCanonicalUrl(`/series/${encodeURIComponent(seriesId)}`, {
...
export default async function SeriesWorksPage({
  params,
  searchParams,
}: PageProps) {
  const [{ id }, query] = await Promise.all([params, searchParams]);
  ...
  const currentPage = pageValidation.page;
  const currentSort = normalizeWorkSort(query.sort);
  const result = await getSeriesPageData(
    seriesId,
    currentPage,
    currentSort
  );
  const { items, totalPages, title, seriesName } = result;
  if (result.dataStatus === "out-of-range") notFound();
  const paginationItems = getPagination(currentPage, totalPages);
  const breadcrumbItems: BreadcrumbItem[] = [
    { name: "ホーム", href: "/" },
    { name: "シリーズ", href: "/series" },
    {
      name: seriesName || `シリーズID ${seriesId}`,
  ...
  const makePageHref = (page: number, sort: WorkSort = currentSort) => {
    ...
    return `/series/${encodeURIComponent(seriesId)}?${queryParams.toString()}`;
  };
